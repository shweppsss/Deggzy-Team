...
- TS-1: 168/168 PASS — App.Runtime IIFE removed from `index.html` and re-implemented as a typed TypeScript module at `/src/core/runtime.ts`. The bundled module is loaded via the deferred `<script type="module" src="/src/main.ts">` tag and re-attached to `window.App.Runtime` so all legacy inline call sites keep working unchanged. **The harness does not exercise Runtime directly** — it mocks `document.addEventListener` / `removeEventListener` and tests the calendar / menu / modal handlers that call those APIs natively. Hence the 168 assertions are unaffected by the Runtime extraction. A future TS-test runner (TS-1.x) will add a dedicated unit-test for the typed Runtime module API.
